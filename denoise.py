...
def unet():
        ...
        conv1  = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv1', use_bias=True,padding="same")(model_input)
        conv1  = KL.LeakyReLU(alpha=0.2)(conv1)
        conv1a = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv1a', use_bias=True,padding="same")(conv1)
        conv1a  = KL.LeakyReLU(alpha=0.2)(conv1a)
        P1     = KL.MaxPooling2D(pool_size=(3,3),strides=(2,2),padding="same",name="max_pooling_1")(conv1a)

        conv2  = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv2', use_bias=True,padding="same")(P1)
        conv2  = KL.LeakyReLU(alpha=0.2)(conv2)
        conv2a = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv2a', use_bias=True,padding="same")(conv2)
        conv2a  = KL.LeakyReLU(alpha=0.2)(conv2a)
        P2     = KL.MaxPooling2D(pool_size=(3,3),strides=(2,2),padding="same",name="max_pooling_2")(conv2a)

        conv3  = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv3', use_bias=True,padding="same")(P2)
        conv3  = KL.LeakyReLU(alpha=0.2)(conv3)
        conv3a = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv3a', use_bias=True,padding="same")(conv3)
        conv3a  = KL.LeakyReLU(alpha=0.2)(conv3a)
        P3     = KL.MaxPooling2D(pool_size=(3,3),strides=(2,2),padding="same",name="max_pooling_3")(conv3a)

        conv4  = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv4', use_bias=True,padding="same")(P3)
        conv4  = KL.LeakyReLU(alpha=0.2)(conv4)
        conv4a = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv4a', use_bias=True,padding="same")(conv4)
        conv4a  = KL.LeakyReLU(alpha=0.2)(conv4a)
        P4     = KL.MaxPooling2D(pool_size=(3,3),strides=(2,2),padding="same",name="max_pooling_1")(conv4a)

        conv5  = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv5', use_bias=True,padding="same")(P4)
        conv5  = KL.LeakyReLU(alpha=0.2)(conv5)
        conv5a = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv5a', use_bias=True,padding="same")(conv5)
        conv5a  = KL.LeakyReLU(alpha=0.2)(conv5a)
        P5     = KL.MaxPooling2D(pool_size=(3,3),strides=(2,2),padding="same",name="max_pooling_1")(conv5a)

        
        up4      = KL.Deconvolution2D(nb_filter=256, nb_row=3, nb_col=3,subsample=(2, 2),name='deconv4',border_mode='same')(P5)
        up4      = KL.LeakyReLU(alpha=0.2)(up4)
        C4       = KL.Concatenate()([P4,up4])
        conv4_u  = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv4_u', use_bias=True,padding="same")(C4)
        conv4_u  = KL.LeakyReLU(alpha=0.2)(conv4_u)
        conv4a_u = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv4a_u', use_bias=True,padding="same")(conv4_u) 
        conv4a_u = KL.LeakyReLU(alpha=0.2)(conv4a_u)

        up3      = KL.Deconvolution2D(nb_filter=256, nb_row=3, nb_col=3,subsample=(2, 2),name='deconv3',border_mode='same')(conv4a_u)
        up3      = KL.LeakyReLU(alpha=0.2)(up3)
        C3       = KL.Concatenate()([P3,up3])
        conv3_u  = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv3_u', use_bias=True,padding="same")(C3)
        conv3_u  = KL.LeakyReLU(alpha=0.2)(conv3_u)
        conv3a_u = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv3a_u', use_bias=True,padding="same")(conv3_u)
        conv3a_u  = KL.LeakyReLU(alpha=0.2)(conv3a_u)
        
 
        up2      = KL.Deconvolution2D(nb_filter=256, nb_row=3, nb_col=3,subsample=(2, 2),name='deconv2',border_mode='same')(conv3a_u)
        up2      = KL.LeakyReLU(alpha=0.2)(up2)
        C2       = KL.Concatenate()([P2,up2])
        conv2_u  = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv2_u', use_bias=True,padding="same")(C2)
        conv2_u  = KL.LeakyReLU(alpha=0.2)(conv2_u)
        conv2a_u = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv2a_u', use_bias=True,padding="same")(conv2_u)
        conv2a_u = KL.LeakyReLU(alpha=0.2)(conv2a_u)
        
        
        up1      = KL.Deconvolution2D(nb_filter=256, nb_row=3, nb_col=3,subsample=(2, 2),name='deconv3',border_mode='same')(conv2a_u)
        up1      = KL.LeakyReLU(alpha=0.2)(up1)
        C1       = KL.Concatenate()([P1,up1])
        conv1_u  = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv1_u', use_bias=True,padding="same")(C1)
        conv1_u  = KL.LeakyReLU(alpha=0.2)(conv1_u)
        conv1a_u = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv1a_u', use_bias=True,padding="same")(conv1_u) 
        conv1a_u = KL.LeakyReLU(alpha=0.2)(conv1a_u)

        up0      = KL.Deconvolution2D(nb_filter=256, nb_row=3, nb_col=3,subsample=(2, 2),name='deconv3',border_mode='same')(conv1a_u)
        up0      = KL.LeakyReLU(alpha=0.2)(up0)
        C0       = KL.Concatenate()([model_input,up0])
        conv0_u  = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv0_u', use_bias=True,padding="same")(C0)
        conv0_u  = KL.LeakyReLU(alpha=0.2)(conv0_u)
        conv0a_u = KL.Conv2D(32, (3, 3), strides=(1, 1), name='conv0a_u', use_bias=True,padding="same")(conv0_u)
